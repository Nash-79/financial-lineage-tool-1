...
Enable each project to store **user-authored context** that guides LLM-assisted lineage extraction and knowledge graph construction. The context is project-scoped (not per-repo or per-file) and is optional.
...
### Internal
- Requires: project storage / project-structure (projects table or equivalent)
- Integrates with: ingestion orchestration and LLM prompt builder

### External
- DuckDB JSON support
- File system storage (for uploaded context files)
