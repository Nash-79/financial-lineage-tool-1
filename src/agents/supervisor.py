...
class LineageQuery:
    ...
    query_type: str = "general"  # column_lineage, table_lineage, transformation, validation
        ...
        tools.append(Tool(
            ...
            parameters={
                ...
                "properties": {
                    ...
                    "search_type": {
                        "type": "string",
                        "enum": ["transformation", "definition", "join", "aggregation"],
        ...
        tools.append(Tool(
            ...
            parameters={
                ...
                "properties": {
                    ...
                    "validation_type": {
                        "type": "string",
                        "enum": ["data_types", "transformations", "completeness", "all"],
        ...
        for path in raw_paths:
            if isinstance(path, list):
                formatted.append({
                    "nodes": [
                        {
                            "name": node.get("name", [""])[0] if isinstance(node.get("name"), list) else node.get("name", ""),
                            "type": node.get("entity_type", [""])[0] if isinstance(node.get("entity_type"), list) else node.get("entity_type", "")
                        }
                        for node in path if isinstance(node, dict)
                    ]
                })
        return formatted
...
class ValidationAgent:
    ...
    async def validate(
        ...
        return {
            ...
            "issues": issues,
            "validation_types_checked": [validation_type] if validation_type != "all" 
    ...
    def _types_compatible(self, source_type: str, target_type: str) -> bool:
        ...
        numeric_types = {"int", "integer", "bigint", "decimal", "float", "double", "numeric"}
