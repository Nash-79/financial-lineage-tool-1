...
def export_graph_for_visualization(output_file: str = "data/graph_viz.json") -> Dict[str, Any]:
    """
...
def export_cypher_queries(output_file: str = "data/cypher_queries.json") -> Dict[str, Any]:
    """
    Export common Cypher queries.
    """
    queries = {
        "get_all_tables": {"description": "Get all table nodes", "cypher": "MATCH (n:Table) RETURN n"},
        "get_all_columns": {"description": "Get all column nodes", "cypher": "MATCH (n:Column) RETURN n"},
        "get_table_columns": {
...
class EmbeddingExporter:
    ...
    async def export_text_embeddings(self, texts: List[str], output_file: str = "data/embeddings_export.json"):
        ...
        for idx, text in enumerate(texts):
            try:
                ...
                embeddings_data.append({
                    ...
                    "embedding_dimension": len(emb), "model": self.embedding_model
                })
    ...
    async def export_sql_embeddings(self, sql_file: str, output_file: str = "data/sql_embeddings.json"):
    ...
    async def export_entity_embeddings(self, output_file: str = "data/entity_embeddings.json"):
        print("[*] Connecting to Neo4j...")
        client = Neo4jGraphClient(
            uri=os.getenv("NEO4J_URI"),
            username=os.getenv("NEO4J_USERNAME"),
        ...
        try:
            ...
            for ent in entities:
                ...
                if ent['properties']:
                    props = ", ".join([f"{k}={v}" for k,v in ent['properties'].items() if k not in ['id', 'name']])
                    if props: text += f" ({props})"
                
                try:
                    emb = await self.generate_embedding(text)
                    embeddings_data.append({
                        "entity_id": ent['id'], "entity_name": ent['name'], 
                        "text_representation": text, "embedding": emb,
                        "embedding_dimension": len(emb), "model": self.embedding_model
                    })
                except Exception as e:
