...
class SQLChunker(BaseChunker):
    ...
    def _chunk_statement(
        ...
        if self.count_tokens(sql) <= self.max_tokens:
            return [self._create_chunk(sql, ChunkType.SQL_STATEMENT, file_path, statement)]
    ...
    def _extract_tables(self, sql: str) -> list[str]:
        ...
        try:
            ...
            for table in parsed.find_all(exp.Table):
                full_name = ".".join(filter(None, [table.catalog, table.db, table.name]))
