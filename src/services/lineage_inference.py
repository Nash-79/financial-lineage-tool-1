...
class LineageInferenceService:
    ...
    async def retrieve_context(
        ...
        try:
            ...
            if project_id:
                ...
                LIMIT $max_nodes
                """
                params = {"scope": scope, "max_nodes": max_nodes, "project_id": project_id}
            else:
                node_query = """
                MATCH (n)
                WHERE (n:File OR n:DataAsset OR n:Column OR n:FunctionOrProcedure)
            ...
            if context["nodes"]:
                ...
                rels_result = self.graph._execute_query(rel_query, {
                    "node_ids": node_ids
                })
            ...
            try:
                ...
                logger.info(f"Retrieved {len(context['code_chunks'])} code chunks from Qdrant")
    ...
    def _validate_proposal(
        ...
        if proposal["source_id"] not in node_ids or proposal["target_id"] not in node_ids:
            return False
...
1. source_id: ID of the source entity (must be from the graph context)
2. target_id: ID of the target entity (must be from the graph context)
...
"""
    ...
    def ingest_proposals(
        ...
        logger.info(f"Successfully ingested {ingested_count}/{len(proposals)} proposals")
