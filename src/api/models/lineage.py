...
class LineageInferenceRequest(BaseModel):
    ...
    scope: str = Field(..., description="File path or scope identifier for context retrieval")
    project_id: Optional[str] = Field(None, description="Optional project ID to filter context")
    max_nodes: int = Field(20, ge=1, le=100, description="Maximum graph nodes to include in context")
    max_chunks: int = Field(10, ge=1, le=50, description="Maximum code chunks to include in context")
