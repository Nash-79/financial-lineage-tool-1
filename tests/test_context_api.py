...
class TestProjectContextAPI(unittest.TestCase):
    ...
    def test_context_lifecycle(self):
        ...
        resp = requests.put(f"{BASE_URL}/projects/{self.project_id}/context", json=new_context)
        self.assertEqual(resp.status_code, 200)
...
"""
        files = {
            'file': ('context.md', md_content, 'text/markdown')
        }
        
        # Upload file
        resp = requests.post(f"{BASE_URL}/projects/{self.project_id}/context/upload", files=files)
