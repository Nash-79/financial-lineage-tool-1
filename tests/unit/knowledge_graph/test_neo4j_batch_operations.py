...
class TestNeo4jBatchOperations(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with patch('src.knowledge_graph.neo4j_client.GraphDatabase.driver', return_value=self.mock_driver):
    ...
    def test_batch_create_entities_single_batch(self):
        ...
        self.mock_session.execute_write.return_value = [Mock(data=lambda: r) for r in mock_result]
    ...
    def test_batch_create_entities_with_retry_on_transient_error(self):
        ...
        entities = [
            {"id": "t1", "entity_type": "Table", "name": "Users"}
        ]
    ...
    def test_batch_create_entities_exhausts_retries(self):
        ...
        self.mock_session.execute_write.side_effect = TransientError("Connection timeout")

        with patch('time.sleep'):  # Skip actual sleep delays
            with patch('src.knowledge_graph.neo4j_client.logger'):  # Suppress logs
                # Should attempt partial failure recovery
                with patch.object(self.client, '_handle_partial_failure', return_value=0):
