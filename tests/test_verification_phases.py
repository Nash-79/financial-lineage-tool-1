...
async def test_restart_endpoint():
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            ...
            assert data["status"] == "restarting", f"Expected status='restarting', got {data['status']}"
            ...
            print("  - Test 2.3: Verify container restarts within 10 seconds (requires Docker)")
...
async def test_activity_tracking():
    ...
    if not activity_log.exists():
        ...
        print("  This is expected if activity tracking middleware is not yet configured")
    ...
    try:
        ...
        for i, line in enumerate(lines[:5]):  # Check first 5 entries
            try:
                ...
                print(f"  Entry {i+1}: type={event.get('type', 'unknown')}, timestamp={event.get('timestamp', 'missing')}")
...
async def test_pydantic_models():
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            ...
            key_models = ["ChatRequest", "ChatResponse", "ProjectCreate", "RepositoryCreate"]
            ...
            assert docs_response.status_code == 200, f"Expected 200, got {docs_response.status_code}"
